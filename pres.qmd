...
::: {.incremental .medium-par}
- **Comparamos los dataset de producción (coyuntura) y de referencia (entrenamiento)**:
  - aplicando varias métricas de drift implementadas por la librería open-source [Evidently](https://docs.evidentlyai.com/metrics/customize_data_drift).
    - a nivel de cada clase (predicción / ground truth) y dataset.
  - aplicando otras métricas creadas como proxy para estimar novedad, incerteza y diversidad de los registros.
...
| Report Prediction Drift            | drift en las **classes predichas por del modelo**. | dataset |
| Report Prediction Drift vs Truth   | drift en las **classes predichas por del modelo en el dataset de producción** vs **classes verdaderas en el dataset de referencia**. | dataset |
| Report Prediction Class Prob Drift | drift en las **probabilidades de las classes predichas por del modelo**. |  clase + dataset |
| Priority Analysis | priority score basado en **novedad, incerteza y diversidad**. | registro |
...
| Test / Report                      | Método para evaluar drift score | Rango | Threshold | 
|:-|:-|:-|:-|
| Report Text Drift                  | - **Wasserstein distance**: modelos a 1 dígito. <br> - **Kolmogorov–Smirnov (K-S) test**: modelos a 2 dígitos. |  [0, +inf] <br> ?? | drift ≥ 0.1 <br> drift < 0.05
| Report Embedding Drift             | - **Clasificador binario** para distinguir entre los embeddings calculados desde ambos datasets. <br> - Retorna valores de **ROC AUC como *drift_score***. | [0, 1] | drift > 0.55
| Report Prediction Drift            | - **Jensen-Shannon distance**: basada en el método **Kullback-Leibler Divergence**, mide distancia entre dos distribuciones de probabilidad calculando la raíz cuadrada de la **Jensen-Shannon divergence**. <br> - Intuición: mide un *promedio* de todos los cambios en las frecuencias relativas de las categorías. | [0, 1] | drift ≥ 0.1
| Report Prediction Drift vs Truth   | - **Jensen-Shannon distance**.     | [0, 1] | drift ≥ 0.1 
| Report Prediction Class Prob Drift | - **Wasserstein distance**: modelos a 1 dígito. <br> - **Kolmogorov–Smirnov (K-S) test**: modelos a 2 dígitos. |   [0, +inf] <br> ?? | drift ≥ 0.1 <br> drift < 0.05
| Priority Analysis | - novelty_score <br> - uncertainty_score <br> - diversity_score <br> - priority_score: suma ponderada de novelty_score, uncertainty_score, diversity_score. | ?? | ??
...
::: {.incremental .medium-par}
- **5) Priority Analysis:** **LORE**....
  - Novelty score: métrica obtenida combinando la similitud coseno (cosine similarity) y la mínima distancia euclidiana normalizada calculadas para cada embedding del dataset de producción respecto a todos los embeddings del dataset de referencia...
  - Uncertainty score: estima incerteza de registros en base a...
  - Diversity score: métrica obtenida...kmeans...embeddings...distancia a centros de clústers...
:::



## [Arquitectura de Monitoreo | Model monitoring | Combined metric]{.big-par}

::: {.incremental .medium-par}
- **Priority score:**
  - Suma ponderada de novelty_score, uncertainty_score, diversity_score.
- **Drift score:**
  - Suma ponderada de multiples drift scores: embeddings, prediction, prediction vs truth, prediction class probability.
- **Revision score:**
